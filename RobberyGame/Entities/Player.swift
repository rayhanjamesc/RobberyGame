...
class Cat: SKSpriteNode {
    init() {
        // Load the image from your asset catalog
        let texture = SKTexture(imageNamed: "Cat_Idle_1") // Replace "Bear" with the name of your image asset
        let originalSize = texture.size()
        let scaledSize = CGSize(width: originalSize.width, height: originalSize.height)
    ...
    func idleState() {
        var textures: [SKTexture] = []
        for i in 1 ... 2 {
            let texture = SKTexture(imageNamed: "Cat_Idle_\(i)")
            textures.append(texture)
        }
    ...
    func walkingState() {
        var textures: [SKTexture] = []
        for i in 1 ... 4 {
            let texture = SKTexture(imageNamed: "Cat_Walk_\(i)")
            textures.append(texture)
        }
